...
export default function Treasurer() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const [churchData, memberData, offeringData, categoryData] = await Promise.all([
          api.getChurches(),
  ...
  return (
    <div className="p-4 space-y-6 max-w-7xl mx-auto pb-20">
      {/* Header */}
      <div className="bg-emerald-800 text-white p-6 rounded-3xl shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <button
            ...
            className="flex-1 md:flex-none bg-white/20 hover:bg-white/30 px-4 py-2 rounded-xl transition-all flex items-center justify-center gap-2 text-xs font-bold"
          >
            <FileText size={16} /> PDF Report
          </button>
          <button
            onClick={() => generateExcel("Offerings", offerings)}
            className="flex-1 md:flex-none bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-xl transition-all flex items-center justify-center gap-2 text-xs font-bold shadow-lg shadow-emerald-900/20"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="p-6 bg-white border border-slate-100 rounded-[32px] shadow-sm flex flex-col gap-2">
          ...
          <h4 className="text-2xl font-bold text-slate-800">{offeringCategories.length}</h4>
        </div>
        <div className="p-6 bg-white border border-slate-100 rounded-[32px] shadow-sm flex flex-col gap-2">
          ...
          <h4 className="text-2xl font-bold text-slate-800">
            {new Intl.NumberFormat('en-TZ', { style: 'currency', currency: 'TZS', maximumFractionDigits: 0 }).format(offerings.length > 0 ? totalOfferings / offerings.length : 0)}
          </h4>
        </div>
        <div className="p-6 bg-white border border-slate-100 rounded-[32px] shadow-sm flex flex-col gap-2">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
          ...
          <form onSubmit={handleAddOfferingCategory} className="flex gap-2">
            ...
            <button
              type="submit"
              className="px-6 bg-emerald-700 text-white rounded-xl font-bold hover:bg-emerald-800 transition-all"
            >
              <Plus size={20} />
            </button>
          </form>
        ...
        {/* Member List for Offering Registration */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
          <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
            ...
            {members
              ...
              <div key={member.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-emerald-700 text-white text-xs font-bold rounded-xl hover:bg-emerald-800 transition-all flex items-center gap-2"
                >
                  <DollarSign size={14} /> Register Offering
                </button>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-emerald-800 p-8 text-white relative">
              ...
              <p className="text-emerald-100 opacity-80">Registering offering for this member</p>
            </div>
            <form onSubmit={handleAddOffering} className="p-8 space-y-4">
